...
class TempFileInfo:
    ...
    def __init__(self, path: str, service: str = "pandoc", auto_cleanup: bool = True):
        self.path = path
...
class TempFileManager:
    ...
    def __init__(self, base_dir: str = DEFAULT_TEMP_DIR, service: str = "pandoc"):
        """
...
def get_temp_manager(base_dir: str = DEFAULT_TEMP_DIR) -> TempFileManager:
    """
    ...
    if _manager_instance is None:
        _manager_instance = TempFileManager(base_dir=base_dir, service="pandoc")
...
@contextmanager
def temp_file_manager(base_dir: str = DEFAULT_TEMP_DIR):
    """
    Context manager for temporary file management.

    Usage:
        with temp_file_manager() as manager:
            temp_file = manager.create_temp_file(b"content")
            # Use temp_file
        # Automatic cleanup happens here
    """
    manager = TempFileManager(base_dir=base_dir, service="pandoc")
    try:
        yield manager
    finally:
        manager.cleanup_all()
...
def save_content_to_temp_file(content: bytes, filename: str) -> str:
    """
